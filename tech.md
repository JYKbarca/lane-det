...
这个项目是一个基于 Anchor 的车道线检测工程，核心任务是对 TuSimple 数据格式的道路图像进行车道线预测，并输出可用于官方评估的结果文件。

当前主线方法可以概括为：

`TuSimple 数据集 -> 固定 y 采样的车道线表示 -> Anchor 生成与匹配 -> ResNet18 + FPN + Anchor Head -> 车道线解码 -> TuSimple 格式导出 -> Accuracy / FP / FN 评估`

项目当前核心模块分布如下：

- 配置文件：`configs/`
- 数据集与变换：`lane_det/datasets/`
- Anchor 生成与标签分配：`lane_det/anchors/`
...
6. 为当前输入尺寸生成 Anchor 集
7. 将 GT 车道线与 Anchor 做匹配，生成分类和回归监督标签
8. 将样本组装成 batch，送入检测模型
9. 模型输出 Anchor 分类分数与车道偏移回归量
10. 使用分类损失和回归损失训练模型
...
`原始 JSON 标注 -> 统一车道线表示 -> 数据增强/缩放 -> Anchor 集生成 -> Anchor 与 GT 匹配 -> 训练监督标签`
...
这个项目中的 Anchor 不是目标检测里的矩形框，而是一组“候选车道线模板”。
...
Anchor 生成位于 `lane_det/anchors/anchor_generator.py`，主要分两类。
...
所以这类 Anchor 适合表示那些“从图像底部进入画面”的车道线。
...
`原始车道线标注 -> 标准化车道线表示 -> Anchor 匹配 -> 分类标签 + 偏移回归标签`
...
`已经完成标签分配的 batch 数据 -> 如何进入神经网络 -> 网络内部如何提取特征 -> 如何沿 Anchor 轨迹取特征 -> 如何输出分类分数和回归偏移`
...
“把这些样本送进网络，并把图像特征转成每条 Anchor 的分类与几何修正预测。”
...
`images -> ResNet18 提取多层特征 -> FPN 融合成多尺度金字塔特征 -> 沿 Anchor 轨迹池化特征 -> 分类头输出每条 Anchor 的存在分数 -> 回归头输出每条 Anchor 在各 y 位置上的偏移量`
...
这些统计的作用非常重要，因为这个项目是 Anchor-based 结构，训练能否稳定，很大程度取决于标签分配是否合理。
...
这对于 Anchor-based 车道线检测尤其关键，因为候选 Anchor 往往大量为负样本。
...
`训练脚本利用分类损失、逐点回归损失和可选的线级相似性损失，对 Anchor-based 车道线检测模型进行联合优化，并在每个 epoch 后通过真实解码和 TuSimple 指标验证模型效果，再保存阶段性和最优权重。`
...
对于 Anchor-based 车道线检测而言，很多训练问题其实不是模型造成的，而是：
...
`数据准备 -> 标注解析 -> Anchor 生成与匹配 -> 多尺度特征提取 -> 沿 Anchor 轨迹采样 -> 分类与逐点偏移回归 -> 两阶段细化 -> 多损失联合训练 -> 验证集真实解码评估 -> checkpoint 保存 -> 离线推理 -> lane NMS 与平滑 -> TuSimple 格式导出 -> Accuracy / FP / FN 评估 -> 可视化分析与实验迭代`
